...
export function ProcedurePreview({procedure, dense = false}: {
    procedure: Procedure,
    dense?: boolean
}) {
        ...
        ${procedure.type === ProcedureType.Guideline ? "bg-tremor-brand-faint border-2 border-dotted border-tremor-brand-emphasis" : ""}`}>
            <Flex flexDirection={"col"} alignItems={"start"} className={"gap-2"}>
                <Text className={`${dense ? "font-semibold text-lg" : "font-bold text-xl"} text-start`}>
                    {procedure.title}
                ...
                {procedure.active && !dense && <Text className={"three-lines text-start text-xs opacity-80"}>
